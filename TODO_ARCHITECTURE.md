...
- [x] Déplacer `App\Models\User\User` -> `Lahatre\Iam\Models\User`
- [x] Déplacer `App\Models\User\UserProfile` -> `Lahatre\Iam\Models\UserProfile`
...
- [x] Déplacer `App\Models\Company\Company` -> `Lahatre\Iam\Models\Company` (ou un nouveau module `organization`).
- [x] Déplacer `App\Models\Company\CompanyMember` -> Idem.
