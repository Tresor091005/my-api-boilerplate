...
it('does not silently apply the default quota when organization settings are missing', function (): void {
    OrganizationSetting::query()
        ->where('organization_id', $this->organization->id)
        ->firstOrFail()
        ->delete();
    ...
    $this->post('/v1/library/files?response=resource', [
        'files' => [UploadedFile::fake()->createWithContent('missing-settings.txt', 'content')],
...
it('renames a restored folder instead of replacing a newer folder with the same name', function (): void {
    ...
    expect(Folder::query()->findOrFail($folder->id)->name)->toBe('Documents (1)');
});

it('uses the next restored folder suffix when earlier suffixes already exist', function (): void {
    $folder = Folder::factory()->create([
        'organization_id' => $this->organization->id,
        'name'            => 'Documents',
    ]);
    ...
    $this->deleteJson("/v1/library/folders/{$folder->id}")->assertNoContent();
    Folder::factory()->create(['organization_id' => $this->organization->id, 'name' => 'Documents']);
    Folder::factory()->create(['organization_id' => $this->organization->id, 'name' => 'Documents (1)']);

    $this->postJson("/v1/library/files/{$fileId}/restore?response=resource")->assertOk();

    expect(Folder::query()->findOrFail($folder->id)->name)->toBe('Documents (2)');
});

it('does not restore a folder when its parent has reached the maximum width', function (): void {
    $folder = Folder::factory()->create(['organization_id' => $this->organization->id]);
    $fileId = (string) $this->post('/v1/library/files?response=resource', [
        'folder_id' => $folder->id,
        'files'     => [UploadedFile::fake()->createWithContent('restore.txt', 'restore me')],
    ...
    Folder::factory()->count((int) config('library.folders.max_children'))->create([
        'organization_id' => $this->organization->id,
    ]);
