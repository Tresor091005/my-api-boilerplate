...
- Expose Service CRUD under `/v1/catalog/services`. Deliverable templates are
  managed only through the Service payload: their array order defines position,
  existing IDs are updated, new entries omit the ID, and omitted rows are
  deleted.
- Require at least one deliverable template. Keep `handle` and `unit_group_id`
  immutable after creation, and reserve handles after soft deletion.
- Include deliverable templates in API responses only when requested through
  `include=deliverable_templates`.
...
`fulfillment_quantity`, Service-module HTTP endpoints, OTP, notifications,
billing, returns, scheduling, projects, tasks, or portal.
